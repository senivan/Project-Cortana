...
class TestDictReader(unittest.TestCase):
    ...
    take a str of file name as an argument.
    In your test it's not the case which leads to TypeError. Please fix this
    +
    As argument functions dict_reader_tuple and dict_reader_dict 
    ...
    def test_dict_reader_tuple(self):
        ...
        result = dict_reader_tuple('cmudict.txt')
        with open('cmudict.txt', 'r') as f:
            self.assertEqual(expected, result)

    def test_dict_reader_dict(self):
        expected = {"NACHOS": set([("N", "AE1", "CH", "OW0", "Z"),
                                  ("N", "AA1", "CH", "OW0", "Z")])}
        result = dict_reader_dict('cmudict.txt')
        with open('cmudict.txt', 'r') as f:
            self.assertEqual(expected, result)

    def test_dict_invert(self):
        with open('cmudict.txt', 'r') as f:
            expected = {2: {('NACHOS', ('N', 'AE1', 'CH', 'OW0', 'Z')),
                              ('NACHOS', ('N', 'AA1', 'CH', 'OW0', 'Z'))}}
            result = dict_invert(dict_reader_tuple('cmudict.txt'))
            self.assertEqual(expected, result)

            expected = {2: {('NACHOS', ('N', 'AE1', 'CH', 'OW0', 'Z')),
                              ('NACHOS', ('N', 'AA1', 'CH', 'OW0', 'Z'))}}
            result = dict_invert(dict_reader_dict('cmudict.txt'))
            self.assertEqual(expected, result)
