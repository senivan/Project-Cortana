...
# New code:
# Prompt: refactor this code for it to be faster and better
# Comments: Copilot did a good job refactoring the code. The code works perfectly and he made it from the first try.
# ALso code works fastrer than before.
...
def generate_grid():
    ...
    for _ in range(3):
        ...
        for i in range(3):
            if i % 2 == 0:
                small_list.append(random.choice(vowel_list))
            else:
                small_list.append(random.choice(cons_list))
...
'irok', 'komi', 'kori', 'miro', 'moki', 'ovum', 'work', 'worm', 'wouf']
    ...
    with open(f,'r+',encoding='utf-8') as file:
        for line in file:
            line_sliced = line.strip().lower()
            if len(line_sliced)>=4 and line_sliced.count(letters[4])>0:
                if all(line_sliced.count(j) <= letters.count(j) for j in line_sliced):
                    final_lst.add(line_sliced)
...
def get_words_without_user(f, letters):
    all_words_set = set(get_words(f,letters))
    user_words_set = set(get_user_words())
    return list(all_words_set - user_words_set)
...
'from', 'frow', 'irok', 'komi', 'kori', 'miro', 'moki', 'ovum', 'work', 'worm', 'wouf'])
    ...
    """
    final_lst = []
    words_from_dict_set = set(words_from_dict)
    for line in user_words:
        line_sliced = line.strip().lower()
        if len(line_sliced)>=4 and line_sliced.count(letters[4])>0:
            if all(line_sliced.count(j) <= letters.count(j) for j in line_sliced):
                if line not in words_from_dict_set:
                    final_lst.append(line_sliced)
    return final_lst
